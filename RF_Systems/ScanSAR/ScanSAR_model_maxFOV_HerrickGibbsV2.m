...
% Design Freeze code 
% Change from v0:
% The minimum Area constraint is calculated with half the beamwidth in the
% smallest direction
% change from v1:
% Number of sub swath widths limited by angle nu. 
% Number sub swaths defined as 8 based on SNR, FOV trade-off and uncertanity.


%% Choose operational mode: 'stripSAR' or 'scanSAR'
operational_mode = 'scanSAR';
if strcmpi(operational_mode, 'scanSAR')
    Nsub = 7;  % number of subswaths in scanSAR mode (must be >= 1)
else
    Nsub = 1;
end
...
% Also preallocate storage for physical antenna area, bandwidth used, peak power, and PRF
        ...
        for i = 1:N
            for m = 1:M
                ...
                for j = 1:Lr
                    ...
                    % Calculate swath width for each subswath and total swath
                    SW = 0;
                    R_slant_total = 0; 
                    for t = 1:Nsub
                        ...
                        SW_strip = lambda * R_slant / (antenna_width_vec(i)*sind(graz_ang)); % SMAD p.508
                        SW = SW + SW_strip;
...
prf_vec      = reshape(prf_all, numDesigns, 1);   % <-- Flatten PRF array
...
beamwidth = lambda./(1.20);  % Synthetic aperture beamwidth
...
FOV = atand((SW_vec)./(2*Range_vec));  % effective FoV in degrees
...
found_prf_max         = prf_vec(idx_max);   % <-- Retrieve PRF for the max FOV design
...
fprintf('Pulse Repetition Frequency (PRF) = %.2f Hz\n', found_prf_max);  % <-- Print PRF

%% User Query: Find Design Data for a Given FOV Angle and Range
% Define target design parameters:
target_FOV   = 10;     % Target FOV angle in degrees
...
% Assign the design number for later identification (e.g., to reference sigma_ang)
design_number = idx_target;
...
found_prf        = prf_vec(idx_target);   % <-- Retrieve PRF for the queried design

% Print the design data:
fprintf('\n=== USER-QUERIED DESIGN DATA (FOV Angle and Range Query) ===\n');
fprintf('Design Number          = %d\n', design_number);
...
%% New Section: Uncertainty vs. Peak Power for Various FOV-Dependent Designs (p_{peak} \geq 1W)
...
uncertainty_maxFOV  = zeros(numLevels,1);
uncertainty_100km   = nan(numLevels,1);
uncertainty_150km   = nan(numLevels,1);
uncertainty_200km   = nan(numLevels,1);
