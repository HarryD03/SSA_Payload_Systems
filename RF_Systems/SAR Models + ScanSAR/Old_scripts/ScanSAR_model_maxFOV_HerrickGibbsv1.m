...
for j = 1:Lr
    for i = 1:N
        for m = 1:M
            ...
            beamwidth = (lambda/ antenna_width_vec(i))/2;     % beamwidth, taking antenna width as aperature in this direction as its the largest so aperature is the smallest.
            beamwidth = rad2deg(beamwidth);
            R_val = range_vec(j);
            R_val = R_val *cosd(beamwidth); %Slant Range
            A_min = (4*v_rel*lambda*R_val)/c * tand(beamwidth);
        ...
        for i = 1:N
            for m = 1:M
                ...
                for j = 1:Lr
                    ...
                    R_slant = R_val * cosd(beamwidth); %get Slant Range
                    graz_ang = 90 - beamwidth;
